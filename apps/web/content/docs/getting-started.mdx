...
description: "Install scribe-cms, define a schema, write content, and read it back typed — in five steps."
order: 1
---
...
Requirements: Node 20+, Zod v4. Scribe is framework-agnostic — it works with any Node-based stack. `better-sqlite3` is a native module, so keep it external to your bundler (see the [runtime API](/docs/runtime-api) page).
...
The file name (`hello-world`) is the English slug. `publishedAt` is one of the built-in fields available on every type — see [content model](/docs/content-model).
...
`document.frontmatter` is typed from your Zod schema. See the [runtime API](/docs/runtime-api) for the full surface.
...
Scribe finds every page that is missing or stale in French, translates the fields you marked `field.translatable()`, and stores the result in `.scribe/store.sqlite`. **Commit that directory** — never add `.scribe/` to `.gitignore`. Details in [translation](/docs/translation).

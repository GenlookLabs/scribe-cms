...
Translates missing and stale pages with Gemini (needs `GEMINI_API_KEY`). Run without flags in a terminal, it interactively asks for content type, locale preset, and strategy. In scripts, pass flags:
...
| `--concurrency <n>` | `3` | Parallel translation requests. |
...
Interactive runs show a live progress bar with per-item status, running input/output token counts, and an estimated cost in USD. The command exits non-zero if any item failed. Details in [translation](/docs/translation).

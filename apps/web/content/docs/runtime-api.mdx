...
Handles, in order: direct hit → wrong-locale slug redirect → [locale fallback chain](/docs/configuration#locale-fallback-chains) (on by default) → English fallback (when `indexFallback: "en"`). When a fallback locale serves the page, `actualLocale` is that locale and slug correction is chain-aware. Slug migrations and retired documents are handled by `_redirects.json` rules in your proxy/static redirect map, not by `resolve()`. A typical page handler (Next.js shown):
...
For a locale without a translation, the prerendered slug comes from the first [locale fallback chain](/docs/configuration#locale-fallback-chains) locale that has one (then the English slug), so prerendered URLs match what `resolve()` serves.
